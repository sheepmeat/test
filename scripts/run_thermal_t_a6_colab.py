...
def reconstruct_raw_byte_split(parts: Sequence[Path], output_path: Path, *, format_report: Mapping[str, Any], chunk_size: int = 8 * 1024 * 1024) -> dict[str, Any]:
    ...
    return {"path": _logical(output_path), "size_bytes": total, "member_count": member_count, "finalized": True}
...
def run(*, mode: str, drive_raw_root: Path, work_root: Path, drive_output_root: Path, repo_root: Path = ROOT, dry_run: bool = True, include_real_test: bool = False) -> dict[str, Any]:
    ...
    ledger["partitions"]["train"] = {"format_report": format_report, "status": "FORMAT_IDENTIFIED"}
    save_resume_ledger(ledger_path, ledger)
    from datasets.thermal.canonical_converter import COLAB_STAGE2_MODE, ConversionConfig, convert_partition

    staged_train = work_root / "train.zip"
    reconstructed = reconstruct_raw_byte_split(train_parts, staged_train, format_report=format_report)
    ...
    train_summary = convert_partition(config=train_config, repo_root=repo_root, source_archive=staged_train, artifact_dir=output_artifact_root, overwrite=False)
    validation_archive = drive_raw_root / "validation.zip"
    ...
    validation_summary = convert_partition(config=validation_config, repo_root=repo_root, source_archive=validation_archive, artifact_dir=output_artifact_root, overwrite=False)
    ledger["partitions"].update({"train": {"status": "FINALIZED", "summary": train_summary, "reconstructed": reconstructed}, "validation": {"status": "FINALIZED", "summary": validation_summary}})
    ledger["finalized"] = True
    ledger["execution_result_bundle"] = "T-A6_execution_result"
    save_resume_ledger(ledger_path, ledger)
    bundle = build_execution_result_bundle(drive_output_root, startup=startup, status="CONVERSION_COMPLETE", message="synthetic partitions converted with the frozen T-A6 pipeline", ledger=ledger)
    _atomic_json(bundle / "canonical_artifact_registry.json", {"train": train_summary.get("artifact_path"), "validation": validation_summary.get("artifact_path")})
    _atomic_json(bundle / "conversion_status_summary.json", {"train": train_summary.get("status_counts"), "validation": validation_summary.get("status_counts")})
    _atomic_json(bundle / "output_checksums.json", {"train": train_summary.get("artifact_sha256"), "validation": validation_summary.get("artifact_sha256")})
    return {"status": "CONVERSION_COMPLETE", "bundle": _logical(bundle), "startup": startup, "multipart": format_report, "train": train_summary, "validation": validation_summary}
...
def main() -> None:
    ...
    try:
        ...
        print(json.dumps({"status": "BLOCKED", "code": exc.code, "message": exc.detail}, ensure_ascii=False, sort_keys=True, indent=2))
        raise SystemExit(2)
