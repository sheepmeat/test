...
def build_feature_profile() -> Dict[str, Any]:
    ...
    return {
        ...
        "device_adapter_alignment": {
            "evidence_path": "sensors/co2/co2_adapter.py",
            "adapter_window_seconds": 150.0,
            "adapter_window_samples_at_0_2hz": 30,
            "adapter_method": "ENDPOINT_DIFFERENCE",
            "note": (
                "Offline UCI reconstruction locks the temporal duration (150s) and "
                "endpoint-difference method from the active adapter. UCI nominal "
                "cadence is 60s, so the offline history typically spans three "
                "intervals (~179–181s) rather than thirty 5s device samples."
            ),
        },
