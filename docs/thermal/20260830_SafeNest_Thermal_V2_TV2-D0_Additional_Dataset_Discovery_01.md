...
**Human semantics.** The dataset contains 783 clips, approximately 560,000 frames, and about 10.4 hours. Each clip has 0–9 annotated human-object interactions. The paper/project description provisionally lists walking, sitting, and kneeling, with room/furniture/viewpoint variation; whether individual clips provide usable partial visibility or interaction occlusion must be verified from the files. The exact `annotation.json` action vocabulary requires TV2-D1 payload verification. No official fall, lying, or reclining label was verified. This is therefore a normal-behavior/hard-negative source, not a fall-positive source.
...
| [Thermal-IM](https://github.com/ZitianTang/Thermal-IM) | `RGBT_T.mp4`, 288×384, 15 FPS; rendered video, `NON_RADIOMETRIC_THERMAL_INTENSITY`. | 783 clips, ~10.4 h, provisionally reported walking/sitting/kneeling and object interactions; exact `annotation.json` vocabulary requires TV2-D1 payload verification; no verified fall/lying labels. | Actor/room/scene metadata and train/validation/test fields; exact actor count and full isolation not verified. | Public repository download; repository states BSD-3-Clause. | `PROMISING_WITH_ADAPTATION`. D1 must audit actor/scene splits and extract hard-negative windows. |
...
3. **Thermal-IM — `PROMISING_WITH_ADAPTATION`.** Best currently discoverable normal-motion source with a stated dataset license, temporal thermal video, actor/room metadata, and provisionally reported walking/sitting/kneeling interactions. Its exact `annotation.json` action vocabulary requires TV2-D1 payload verification. It lacks fall labels, so its role is hard-negative and temporal-context enrichment rather than positive generation.
