...
- Report: `docs/reports/20260828_Codex_Thermal_B6R_B6R-P4_Public_SDT_Software_Only_Robustness_Failure_Mode_Audit_Report_KO_01.md`
- Date: `2026-08-28`
- Unit: `B6R-P4 — Public SDT FP32 TFLite Offline Robustness & Failure-Mode Audit`
- Status: `PASS_WITH_LIMITATIONS — PUBLIC_DATA_SOFTWARE_ONLY_NON_GATING`
- Contract: `config/thermal/b6r_p4_public_sdt_software_robustness_failure_mode_contract.json`
- Evidence: `datasets/thermal/manifests/B6R-P4_public_sdt_software_robustness_failure_mode/`
- Conclusion: full DEVELOPMENT 8,000개 × 16 deterministic synthetic stress, clean/stress parity 816 pair, invalid-input, numerical integrity와 software determinism audit이 통과했다. locked test/real sensor/Pi access는 모두 0이고 P1/P2/legacy/default/runtime bytes는 불변이다. P3 `BLOCKED_HARDWARE`와 본선 상태는 바뀌지 않는다.
- Historical current-state/P3 보고서는 원문대로 보존한다. P4는 이번 2026-08-28 명시 승인 한 단계에만 적용되며 P5를 승인하지 않는다.
...
| B6R-P4 | `PASS_WITH_LIMITATIONS` | `docs/reports/20260828_Codex_Thermal_B6R_B6R-P4_Public_SDT_Software_Only_Robustness_Failure_Mode_Audit_Report_KO_01.md` | `config/thermal/b6r_p4_public_sdt_software_robustness_failure_mode_contract.json`; `datasets/thermal/manifests/B6R-P4_public_sdt_software_robustness_failure_mode/` | delivery commit은 `git log` 참조 | DEVELOPMENT 8,000×16 software stress, 816-pair parity mismatch 0, numerical/determinism PASS. Public synthetic non-gating이며 P3 hardware/MI48/physical/runtime gate를 바꾸지 않는다. |
...
- B6R-P4는 2026-08-28 사용자 명시 승인으로 `PASS_WITH_LIMITATIONS` 완료됐지만 public DEVELOPMENT synthetic software diagnostic일 뿐이며 P3 hardware blocker, MI48/physical/safety/production gate를 해소하지 않는다.
...
- B6R-P4는 `PASS_WITH_LIMITATIONS — PUBLIC_DATA_SOFTWARE_ONLY_NON_GATING`로 완료됐다. P5는 정의·실행하지 않으며 P4 결과를 P3 PASS나 본선/production readiness로 승격하지 않는다.
...
13. `B6R-P4`는 P2 exact artifact의 DEVELOPMENT-only deterministic synthetic software audit이다. robustness 수치로 gate를 만들거나 sensor/physical robustness를 주장하지 않고, P3 상태·default/runtime/safety 권한을 변경하지 않는다.
