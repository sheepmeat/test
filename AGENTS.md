...
- M-PROT-2 prototype deployable contract (COMPLETE / MERGED): `datasets/mmwave/manifests/M_PROT_2_deployable_artifact_runtime_contract/` — B23 `PYTORCH_FLOAT32_STATE_DICT` frozen by SHA. PR #176 merged; merge commit / Track P base = `97b742dbc9c23d02cf3a74e0d4134ab76b2d0eaa`. Historical machine-readable fields inside that manifest remain frozen evidence and must not be rewritten. Do not treat this bullet as the current Track P gate.
- M-PROT-3 integration runtime wiring (CURRENT Track P gate): `datasets/mmwave/manifests/M_PROT_3_integration_runtime_wiring/` — composes SW-01 PASS admission → causal **time-coverage** window → R1 (owns resampling) → M-PROT-2 B23. PR #177 corrective implementation present; `CURRENT_TRACK_P_GATE=M-PROT-3_PENDING_SOL_REVIEW`. Worker may claim wiring complete; **Sol exact-head review is still required**. `M-PROT-4=NOT_AUTHORIZED_PENDING_M_PROT_3_SOL_REVIEW`. Provisional integration freeze ≠ final scientific selection. Do not start M-PROT-4, merge #177, or use live hardware from this bullet alone.
