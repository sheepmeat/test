...
- Owner-authorized parallel Track P (prototype / integration): `docs/mmwave/20260827_SafeNest_mmWave_V2_Track_P_Track_F_Roadmap_Reconciliation_01.md` and `datasets/mmwave/manifests/MMWAVE_V2_track_p_track_f_roadmap_state/` — `FINAL_SELECTION_MODEL_WORK=BLOCKED` coexists with provisional integration around B23. Prototype semantics: `PROTOTYPE_INTEGRATION_ONLY` / `PROVISIONAL_INTEGRATION_FREEZE` / `NOT_FINAL_SELECTED_MODEL` / `SUBJECT_TO_REPLACEMENT`. Do not reopen M-PV3.8 or authorize M-PV4 from Track P.
...
- M-PROT-2 / M-PROT-3 historical machine-readable fields may still say `PENDING` / `NOT_AUTHORIZED`; that is frozen worker-era evidence. Do not rewrite those historical JSON fields. Current authority is this `AGENTS.md` pointer + the Track P/F reconciliation report.
- M-PROT-3 integration runtime wiring = **COMPLETE / MERGED / PASS_WITH_LIMITATIONS**. PR `#177` merged; authorized exact implementation head `fcbb6c01e792c762e1cee4912fbc454e94a01dad`; merge commit / `origin/main` = `fd9420663666f40e4e7865f3d5497283c889d401`. Manifest: `datasets/mmwave/manifests/M_PROT_3_integration_runtime_wiring/`. Dataflow: SW-01 PASS → causal time-coverage → R1 (owns resampling) → R2 → B23 → WiringReceipt. Not live-hardware validation.
- M-PROT-4 system smoke = **COMPLETE / MERGED / PASS_WITH_LIMITATIONS**. PR `#179` merged; merge commit / confirmed `origin/main` = `8945e5912a00f9fc177408d7993126431c1a958c`. Manifest: `datasets/mmwave/manifests/M_PROT_4_system_smoke/`. Offline/replay/synthetic smoke only — not live MR60 / Pi validation.
- PR `#182` is the M-PROT-5A test-repo predeployment closure package. **CURRENT_TRACK_P_GATE (authoritative on `main` after Sol-authorized merge of `#182`):** `M-PROT-5A = COMPLETE / TEST_REPO_PREDEPLOYMENT_COMPLETE`. Next execution phase = `M-PROT-5B_TEAM_REPO_PI_RUNTIME_PORT` (`NEXT` / **NOT STARTED**; do not start M-PROT-5B from this pointer alone). `M-PROT-5C = DEFERRED / LIVE_HARDWARE_NOT_EXECUTED`. While `#182` remains unmerged, this closure is pending final merge and is not yet live on `main`. After merge, the COMPLETE / NEXT / DEFERRED assignment above is the current Track P gate. Manifest: `datasets/mmwave/manifests/M_PROT_5A_predeployment_closure/`.
...
- Historical M-PROT-4 worker-era wording `M-PROT-4_IMPLEMENTED_PENDING_SOL_REVIEW` is stale once `#179` merged; do not rewrite historical M-PROT-4 JSON fields — current authority is this `AGENTS.md` pointer + M-PROT-5A manifest.
- Historical authorization pointer (superseded): PR `#178` set `M-PROT-4_AUTHORIZED_READY_TO_START` after M-PROT-3 merge; authorization ≠ PASS.
...
- `docs/20260822_SafeNest_mmWave_Public_Multidomain_V2_Development_Roadmap_01.md` remains an active child roadmap for V2 structure; current Track P/F execution authority is the reconciliation report + this `AGENTS.md` pointer (child roadmap does not override lifecycle closure or provisional integration freeze).
