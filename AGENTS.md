...
- Owner-authorized parallel Track P (prototype / integration): `datasets/mmwave/manifests/M_PROT_0_prototype_integration_lane/` — `FINAL_SELECTION_MODEL_WORK=BLOCKED` coexists with `PROTOTYPE_INTEGRATION_MODEL_WORK=READY`. Prototype artifacts are `PROTOTYPE_INTEGRATION_ONLY` / `NOT_FINAL_SELECTED_MODEL` / `SUBJECT_TO_REPLACEMENT`. Do not call prototype nomination a winner or reopen M-PV3.8 / authorize M-PV4 from Track P.
- M-PROT-1 provisional prototype baseline (historical nomination; not a winner; M-PV3.8 panel unchanged): `datasets/mmwave/manifests/M_PROT_1_prototype_candidate_nomination/` — `B23` / Family B / seed 23 / `PROTOTYPE_NOMINATED` / `INTEGRATION_BASELINE` / `PROVISIONAL`. Completed and advanced into M-PROT-2; do not treat this bullet as the current Track P gate.
- M-PROT-2 prototype deployable contract (not final selection): `datasets/mmwave/manifests/M_PROT_2_deployable_artifact_runtime_contract/` — B23 `PYTORCH_FLOAT32_STATE_DICT` frozen by SHA. Worker evidence may record `worker_terminal_result=M_PROT_2_DEPLOYABLE_CONTRACT_FROZEN`, but **Sol review of PR #176 is still required** (`sol_review_status=PENDING_REVIEW` until Sol approves the exact head). `CURRENT_TRACK_P_GATE`: `M-PROT-3=NOT_AUTHORIZED_PENDING_M_PROT_2_SOL_REVIEW`. Do not start M-PROT-3 or claim M-PROT-2 complete/authorized until Sol approves and the reviewed head is merged.
- M-PROT-3 integration runtime wiring (pending Sol review): `datasets/mmwave/manifests/M_PROT_3_integration_runtime_wiring/` — composes SW-01 → causal 30 s window → R1 → M-PROT-2 B23. Worker may record `terminal_verdict=M_PROT_3_INTEGRATION_RUNTIME_WIRING_COMPLETE` with `sol_review_required=true`. `M-PROT-4=NOT_AUTHORIZED_PENDING_SOL_REVIEW`. Provisional integration freeze ≠ final scientific selection. Do not start M-PROT-4 or live hardware from this bullet alone.
