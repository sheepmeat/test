...
- **CURRENT_TRACK_P_GATE:** `M-PROT-5A_AUTHORIZED_ACTIVE` / `TEST_REPO_PREDEPLOYMENT_CLOSURE`. Purpose: finish portable deployment handoff + deterministic bundle/export inside `sheepmeat/test` before any team-repo or Pi work. Manifest: `datasets/mmwave/manifests/M_PROT_5A_predeployment_closure/`.
- M-PROT-5B = `DEFERRED` / `TEAM_REPO_NOT_YET_INSPECTED_FOR_PORT` (future: inspect `jinsu1011/safenest-embedded-competition` Pi-side subtree and port frozen bundle). Do **not** modify the team repository in M-PROT-5A.
- M-PROT-5C = `DEFERRED` / `LIVE_HARDWARE_NOT_EXECUTED` / live Pi + MR60 smoke **NOT_AUTHORIZED** in this phase. Do not SSH, provision, or claim live validation from M-PROT-5A.
