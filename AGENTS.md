...
- **CURRENT_TRACK_P_GATE:** `M-PROT-4_IMPLEMENTED_PENDING_SOL_REVIEW`. Primary system-smoke implementation may be present on `research/mmwave-m-prot-4-system-smoke` / PR pending Sol exact-head review. Worker may claim smoke wiring complete; **Sol review is still required**. `M-PROT-5 = NOT_AUTHORIZED`. Do not merge M-PROT-4 or start M-PROT-5 / live hardware from this bullet alone. Manifest: `datasets/mmwave/manifests/M_PROT_4_system_smoke/`.
- Historical authorization pointer (superseded once implementation PR exists): PR `#178` set `M-PROT-4_AUTHORIZED_READY_TO_START` after M-PROT-3 merge; authorization ≠ PASS.
