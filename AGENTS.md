...
- M-PROT-1 provisional prototype baseline (not a winner; M-PV3.8 panel unchanged): `datasets/mmwave/manifests/M_PROT_1_prototype_candidate_nomination/` — `B23` / Family B / seed 23 / `PROTOTYPE_NOMINATED` / `INTEGRATION_BASELINE` / `PROVISIONAL`. Next Track P phase is M-PROT-2 deployable-artifact freeze, not M-PV3.8 evaluation.
- M-PROT-2 prototype deployable contract (not final selection): `datasets/mmwave/manifests/M_PROT_2_deployable_artifact_runtime_contract/` — B23 `PYTORCH_FLOAT32_STATE_DICT` frozen by SHA; `M_PROT_2_DEPLOYABLE_CONTRACT_FROZEN`. Next Track P phase is M-PROT-3 runtime wiring.
